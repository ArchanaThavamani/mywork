...
public interface ShipmentFeignClient {
    ...
    @GetMapping(value ="/{storeHash}/v2/orders/{orderId}",produces = "application/json")
    ...
    @GetMapping(value ="/{storeHash}/v2/orders/{orderId}/products",produces = "application/json")
            ResponseEntity<String> getOrderProducts(
            @PathVariable("storeHash") String storeHash,
            @PathVariable("orderId") int orderId,
            @RequestHeader("X-Auth-Token") String accessToken
    );

    @GetMapping(value = "/{storeHash}/v2/orders/{orderId}/shipping_addresses",
            produces = "application/json")  
            ResponseEntity<String> getOrderShippingAddresses(
            @PathVariable("storeHash") String storeHash,
            @PathVariable("orderId") int orderId,
            @RequestHeader("X-Auth-Token") String accessToken
    );
