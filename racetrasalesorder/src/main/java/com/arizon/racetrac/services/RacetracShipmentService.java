...
import com.arizon.racetrac.restclient.ShipmentFeignClient;
...
private void postShipmentToBigCommerce(String storeHash, int orderId, List<OCBCOrderProductTableTransaction> orderProducts) {
    try {
        ...
        // --- Step 1: Fetch Order Shipping Addresses ---
        ResponseEntity<String> addressResponse = shipmentFeignClient.getOrderShippingAddresses(storeHash, orderId, accessToken);
        String body = addressResponse.getBody();
        if (body == null || body.isEmpty()) {
            log.error("Empty response received for shipping addresses of order_id={}", orderId);
            return;
        }
        ...
        if (json instanceof JSONArray) {
            shippingAddresses = (JSONArray) json;
            ...
            log.error("Unexpected response format for shipping addresses: {}", body);
            return;
        }
 
        if (shippingAddresses.isEmpty()) {
            log.error("No shipping addresses found for order_id={} in BigCommerce. Cannot create shipment.", orderId);
            return;
        }
 
        // --- Step 2: Extract order_address_id ---
        ...
        // --- Step 2.5: Fetch order products from BigCommerce ---
        ResponseEntity<String> bcOrderProductsResponse = shipmentFeignClient.getOrderProducts(storeHash, orderId, accessToken);
        JSONArray bcOrderProducts = new JSONArray(bcOrderProductsResponse.getBody());
 
        // --- Step 3: Prepare shipment items ---
        ...
        for (OCBCOrderProductTableTransaction product : orderProducts) {
            ...
            if (matched.isPresent()) {
                Map<String, Object> item = Map.of(
                    "order_product_id", matched.get().getInt("id"), // ✅ order_product_id from BC
        ...
        Map<String, Object> payload = Map.of(
            ...
            "shipping_provider", "",       // empty string is valid if custom
            "tracking_carrier", "",
            "comments", "Shipment created via integration"
        );
 
        // --- Step 5: Create shipment in BigCommerce ---
    ...
    private void markOrderCompleted(String storeHash, int orderId) {
    try {
        String accessToken = "8tu4fs23u4456nj2jrexvz02mu0nkq5";
        Map<String, Object> payload = Map.of("status_id", 11);
        ...
        if (response.getStatusCode().is2xxSuccessful()) {
            ...
            log.error("Failed to update order status for order {}. HTTP {} - {}",
